...
CREATE
OR REPLACE FUNCTION buyCart (p varchar)
RETURNS table (
        idProduct integer,
        isAvailable boolean
) AS $$
...
BEGIN
    return query select c.idProduct, 5 <= p.quantity as dispo from cart c, product p where c.pseudo = $1 AND c.IdProduct = p.IdProduct ORDER BY dispo;
END;
